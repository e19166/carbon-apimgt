...
public class ComplianceMgtDAOImpl implements ComplianceMgtDAO {
    ...
    public List<String> getNonCompliantArtifacts(ArtifactType artifactType, String organization)
        ...
        Set<String> artifactRefIds = new HashSet<>();
        try (Connection connection = APIMGovernanceDBUtil.getConnection()) {
            if (RulesetMgtDAOImpl.isComplianceAffectingSeverityColumnPresent(connection)) {
    ...
    /**
     * Decide whether the violation on the current row of a severity aware query affects compliance.
     * <p>
     * The row carries the severity of the violated rule together with the severities configured for its ruleset. The
     * comparison is done here rather than in SQL, because matching a severity against a comma separated column needs
     * string functions that differ between database vendors.
     *
     * @param resultSet Result set positioned on a row of a severity aware query
    ...
    /**
     * Collect a column from the rows of a severity aware query whose violation affects compliance
     *
     * @param prepStmnt Prepared statement ready to execute
     * @param column    Column to collect
     * @param collected Set the values are added to
