...
public class GovernancePolicyMgtDAOImpl implements GovernancePolicyMgtDAO {
     ...
     * must never be reported as a missing column.
     *
    ...
    private static APIMGovernanceException severityColumnException(SQLException e,
        ...
        if (isMissingSeverityColumn(e)) {
            ...
            return new APIMGovernanceException(APIMGovExceptionCodes.PER_POLICY_SEVERITY_FILTERING_UNAVAILABLE, e,
    ...
    /**
     * Log the remedy when a read failed because the optional column is missing
     * <p>
     * A read cannot be answered without the column and has nothing safe to substitute, so it still fails with the
     * code its own path uses. What it must not do is fail silently about the cause: without this line an operator
     * sees only a generic retrieval failure and no indication that one ALTER TABLE fixes it.
     *
     * @param e Failure from the read
     */
    private static void warnIfSeverityColumnMissing(SQLException e) {
